...
##                ser.write('0') #arudno止める
     ...
     #1分後の処理
        if cont == 10 :		
            ...
            csv_count = sum(1 for line in open('realtime.csv'))
            #表示用のcsvファイル作成
            if csv_count > 60 :
                os.system("tail -60 realtime.csv > graphview.csv")
            readTime = datetime.datetime.today()
            dy=time.strftime('%y/%m/%d %H:%M',time.localtime())
            f.write('%s , %.2f, %.2f, %.3f, %s\n' % (dy,realtemp,humidity,gus,cond))
            f.close()
         ...
         ##   sendMail()
            ...
            #グラフ表示用のrealtime.csvをアップロード
            scp_cmd_r = "scp -i okn_ubu1.pem graphview.csv ubuntu@54.92.62.157:/var/www/html/aba_chart/HighChart/"
            os.system(scp_cmd_r)
    ...
    os.system(scp_cmd)
    #ファイル初期化
    os.system("cat graphview.csv > realtime.csv")
    print "end"
